...
When you get back into Godot, create a new scene. In that scene make your root node a base [Node](https://docs.godotengine.org/en/stable/classes/class_node.html) type. Call it whatever you'd like.

After that create a [FuncGodotMap](ref_func_godot_map.html) node as a child of this node.
...
We went over most of these properties in the [Textures Chapter](guide_textures.html), so we'll just go over the the ones we didn't cover: **Inverse Scale Factor**, **Entity Fgd**, **UV Unwrap Texel Size**, and **Use Groups Hierarchy**.
